...
import { ChevronLeft, User, Book, CreditCard, Calendar } from 'lucide-react';
...
export const StudentDetailsPage: React.FC = () => {
    ...
    if (!data) return <Layout><div>Student not found</div></Layout>;
            ...
            <div className="max-w-6xl mx-auto">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
                    ...
                    <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-100">
                        ...
                        <p className="text-xl font-bold text-blue-600">{stats.activeLibraryLoans.length}</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-lg shadow-sm border border-gray-100 overflow-hidden">
                    <div className="flex border-b">
                        <button
                            className={`px-6 py-3 font-medium text-sm ${activeTab === 'profile' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 hover:text-gray-900'}`}
                            onClick={() => setActiveTab('profile')}
                        >
                            Profile Details
                        </button>
                        <button
                            className={`px-6 py-3 font-medium text-sm ${activeTab === 'fees' ? 'border-b-2 border-primary text-primary' : 'text-gray-600 hover:text-gray-900'}`}
                    ...
                    <div className="p-6">
                        ...
                        {activeTab === 'fees' && (
                            <div>
                                ...
                                {financials.length === 0 ? <p className="text-gray-500">No fee history found.</p> : (
                                    <ul>
                                        {financials.map((fee: any) => (
                                            <li key={fee._id}>...</li>
                                        ))}
                                    </ul>
                                )}
